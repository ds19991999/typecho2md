...
简要记录一下常用的命令行程序，方便以后查阅

## ffmpeg
不少命令行都需要`ffmpeg`，安装一下
```
...
## aria2
[`传送门`](https://www.creat.kim/archives/42/)
...
### 下载安装
```
wget https://github.com/iikira/BaiduPCS-Go/releases/download/v3.5.6/BaiduPCS-Go-v3.5.6-linux-amd64.zip && unzip BaiduPCS-Go-v3.5.6-linux-amd64.zip && rm BaiduPCS-Go-v3.5.6-linux-amd64.zip && mv BaiduPCS-Go-v3.5.6-linux-amd64 BaiduPCS-Go
```
...
cd BaiduPCS-Go
mv BaiduPCS-Go bd
echo "export PATH="$PATH:/root/BaiduPCS-Go" >> /etc/profile
echo "export PATH="$PATH:/root/BaiduPCS-Go" >> ~/.profile
echo "export PATH="$PATH:/root/BaiduPCS-Go" >> ~/.bashrc
```
### 简单使用
详见文档：[BaiduPCS-Go 
](https://github.com/iikira/BaiduPCS-Go/blob/master/README.md). 成功登录之后进入命令行模式就不需要输入前面的`bd`.
```
# 进入BaiduPCS-GO命令行模式
bd
...
# cookie登录
bd login -bduss=<BDUSS>

# 获取网盘配额
bd quota

# 下载文件
# 默认保存到/root/BaiduPCS-Go/download
bd d 文件或目录

# 上传文件
bd u 本地文件或目录 本地文件或目录 ... 网盘目录

# 搜索文件
bd search -path=/ -r 关键字
```

## gdrive
> github: https://github.com/gdrive-org/gdrive

下载：[`传送门`](https://docs.google.com/uc?id=0B3X9GlR6EmbnQ0FtZmJJUXEyRTA&export=download)
```
...
sudo python3 -m pip install onedrivecmd
```
### 使用
```
...
[OPTIONS]
-conf="~/.onedrive.json" #配置文件
-h #帮助
-hack #用SDK上传/aria2下载
-recursive=false #递归列目录，速度比想象中慢得多
-chunk=62914560 #上传分片大小，必须是320K的倍数，不能超过60MB
-url=False #只列出文件下载URL
```

## Pixiv
> github：https://github.com/Tsuk1ko/pxder

之前写了：[传送门](https://www.creat.kim/archives/43/)
...
mv Wudao-dict wudao-dict && cd ./wudao-dict/wudao-dict
sudo bash setup.sh
```
### 使用
```
...
wd Chinese
```
![](https://i.loli.net/2019/04/10/5cad46b77ce44.png)

## BiliBili
> github: https://github.com/Henryhaohao/Bilibili_video_download

### 下载安装
```
sudo apt-get install python3-pip
#可能需要ffmpeg 见：https://www.moerats.com/archives/848/
git clone https://github.com/Henryhaohao/Bilibili_video_download.git /root/bilibili
cd /root/bilibili && pip3 install -r requirements.txt
...
# 来自： https://www.jianshu.com/p/fa4b0724d66c
```

## Annie
支持主流视频网站下载，如`抖音 哔哩哔哩 youtube 爱奇艺 tumblr 网易云音乐 腾讯视频` ... 并且可以作为常见的直链下载器，跨平台，支持`windows`（使用[`scoop`](https://github.com/lukesampson/scoop)安装）.

> github: https://github.com/iawia002/annie#installation
...
#下载youtube
annie url
eg:annie https://www.youtube.com/watch?v=dQw4w9WgXcQ
#显示所有可用的视频质量
...
#下载bilibili番剧 -p选项下载番剧所有集数 -i查看视频流质量
annie -i -p https://www.bilibili.com/bangumi/play/ss26771

#支持代理 -x指定代理
annie -x http://127.0.0.1:7777 -i https://www.youtube.com/watch?v=dQw4w9WgXcQ
